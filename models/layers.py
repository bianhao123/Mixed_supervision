...
class Attention(nn.Module):
    ...
    def forward(self, x, padding_mask=None):
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, N,
                                               self.head_dim * self.num_heads)  # [B, N, d]
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, head_dim=None, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.attn = Attention(
            dim, num_heads=num_heads, head_dim=head_dim, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)  # [B, N, C] -> [B, N, dim]
...
class MHABlock(nn.Module):
    ...
    def forward(self, x, padding_mask=None):
        x = x + self.drop_path(self.attn(self.norm1(x *
                               self.skip_lam), padding_mask)) / self.skip_lam
        return x
...
class FFNBlock(nn.Module):
    ...
    def forward(self, x):
        x = x + \
            self.drop_path(
                self.mlp(self.norm2(x * self.skip_lam))) / self.skip_lam
        return x
...
class PatchEmbed4_2(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        self.conv1 = nn.Conv2d(in_chans, 64, kernel_size=7,
                               stride=2, padding=3, bias=False)  # 112x112
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(64, 64, kernel_size=3,
                               stride=1, padding=1, bias=False)  # 112x112
    ...
    def forward(self, x):
        ...
        x = self.proj(x)  # [B, C, W, H] [B, emded_dim, 14, 14]
